          ...
          <div class="space-y-6 text-base leading-7 text-gray-600">
              ...
              <ol role="list" class="flex items-center space-x-4">
                ...
                <li>
                  <div class="flex items-center">
                    ...
                    <a href="{{ url()->previous() }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">Document</a>
                  </div>
                </li>
                ...
                <li>
                  <div class="flex items-center">
                    ...
                    <a href="#" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700" aria-current="page">Edit</a>
                  </div>
                </li>
              </ol>
            </nav>
            ...
            <div class="mt-12">
              <form wire:submit.prevent="save" enctype="multipart/form-data" class="grid grid-cols-1 font-medium gap-y-6 sm:grid-cols-2 sm:gap-x-8">
                ...
                <div class="space-y-1 sm:col-span-2">
                    <label for="class" class="text-sm">Type :</label><br>
                    <x-select wire:model.lazy="editing.class" id="class" class="w-full border">
                        <option value="">-Select document type-</option>
                        @foreach (App\Models\Doc::TYPE as $value => $label)
                ...
                <div x-show="finalize" x-transition class="p-4 bg-gray-100 rounded-lg sm:col-span-2">
                    ...
                    <div class="space-y-1 sm:col-span-2">
                        <label for="office_id" class="text-sm">Refer to Office :</label><br>
                        <x-select wire:model.lazy="action_taken.office_id" id="office_id" class="w-full">
                            <option value="">-Select Registered Offices-</option>
                            @foreach (App\Models\Office::get() as $label)
                                <option value="{{ $label->id }}">{{ $label->name }}</option>
                            @endforeach
                        </x-select>
                        @error('action_taken.office_id')<x-comment class="text-red-500">*{{ $message }}</x-comment>@enderror
                    </div>

                    <div class="space-y-1 sm:col-span-2">
                        <label for="action" class="text-sm">Action Taken :</label>
                        <div class="mt-1">
                            <textarea wire:model.lazy="action_taken.action" rows="4" id="action" class="block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                        </div>
                        @error('action_taken.action')<x-comment class="text-red-500">*{{ $message }}</x-comment>@enderror
                    </div>

                    <div class="space-y-1 sm:col-span-2">
                        <label for="action_remarks" class="text-sm">Status/Remarks :</label>
                        <div class="mt-1">
                            <textarea wire:model.lazy="action_taken.remarks" rows="4" id="action_remarks" class="block w-full border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                        </div>
                        @error('action_taken.remarks')<x-comment class="text-red-500">*{{ $message }}</x-comment>@enderror
                    </div>
                ...
                <div class="sm:col-span-2">
                    <div class="flex justify-end">
                        ...
                        <x-button wire:click="$toggle('is_draft')" x-on:click="finalize = !finalize" type="button" x-bind:disabled="disabled" ::class="disabled ? '' : 'bg-indigo-600 hover:bg-indigo-400'" class="ml-3">
