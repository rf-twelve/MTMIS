            ...
            <div class="min-w-full overflow-hidden overflow-x-scroll align-middle shadow sm:rounded-lg">
                <x-table>
                    <x-slot name="head">
                        ...
                        <x-table.head class="w-10 px-2 py-1" sortable wire:click="sortBy('tn')"
                            :direction="$sortField === 'tn' ? $sortDirection : null">TRACKING NUMBER
                        </x-table.head>
                        <x-table.head class="px-2 py-1">TITLE</x-table.head>
                        <x-table.head class="px-2 py-1">ORIGIN</x-table.head>
                        <x-table.head class="px-2 py-1">NATURE</x-table.head>
                        <x-table.head class="px-2 py-1">FOR</x-table.head>
                        <x-table.head class="px-2 py-1">OFFICE</x-table.head>
                        ...
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}" class="text-gray-600">
                            ...
                            <x-table.cell class="max-w-2xl">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('Document Overview',['user_id'=>Auth::user()->id,'id'=>$item->id]) }}" class="px-2 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 shadow-sm rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
