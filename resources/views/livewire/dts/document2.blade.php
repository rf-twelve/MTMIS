                    ...
                    <ol role="list" class="flex items-center space-x-4">
                      ...
                      <li>
                        <div class="flex items-center">
                          ...
                          <a href="#" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">Documents</a>
                        </div>
                      </li>
                ...
                <x-table>
                    <x-slot name="head">
                        ...
                        <x-table.head class="px-2 py-1" sortable wire:click="sortBy('tn')"
                            :direction="$sortField === 'tn' ? $sortDirection : null">DOC #</x-table.head>
                        <x-table.head class="w-10 px-2 py-1" sortable wire:click="sortBy('date')"
                            :direction="$sortField === 'date' ? $sortDirection : null">DATE RECEIVED
                        </x-table.head>
                        <x-table.head class="px-2 py-1" sortable wire:click="sortBy('received_by')"
                            :direction="$sortField === 'received_by' ? $sortDirection : null">RECEIVED BY
                        </x-table.head>
                        <x-table.head class="w-1/4 px-2 py-1" sortable wire:click="sortBy('title')"
                            :direction="$sortField === 'title' ? $sortDirection : null">TITLE/ORIGIN/NATURE OF DOCUMENT
                        </x-table.head>
                        <x-table.head class="px-2 py-1" sortable wire:click="sortBy('remarks')"
                            :direction="$sortField === 'remarks' ? $sortDirection : null">REMARKS
                        </x-table.head>
                        <x-table.head class="px-2 py-1">RECIPIENT
                        </x-table.head>
                    ...
                    <x-slot name="body">
                        ...
                        <x-table.row wire:loading.class.delay="opacity-50" wire:key="row-{{ $item->id }}" class="text-gray-600">
                            ...
                            <x-table.cell>
                                @if (count($item->audit_trails) > 0)
                                {{-- <p>Office: {{$item->audit_trails->office_recipient}}</p>
                                <p>Person: {{$item->audit_trails->action_by}}</p> --}}
                                @else
                                <x-comment>(N/A)</x-comment>
                                @endif

                            </x-table.cell>
                            <x-table.cell class="max-w-2xl">
                                <div class="flex justify-center space-x-2">
                                    <a href="{{ route('Document Overview',[$item->id]) }}" class="px-2 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 shadow-sm rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
